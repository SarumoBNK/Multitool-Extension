...
function playAgain(song) {
    stopAllSounds();
    playlist[song] = player.play(paths[song], (err) => {
        if (err) {
...
function stopAllSounds(song = "undefined") {
...
function keepPlayingOrStartPlaying(song) {
...
function message(message = "Extension successfully started!") {
    vscode.window.showInformationMessage(message);
}
function joke(countProblems, type_of_humor) {
    if (countProblems === 52) {
...
let isRunning = false;
function startTimer(updateWebview) {
    if (isRunning)
        return;
    isRunning = true;
...
function activate(context) {
    ...
    const TimerDe = vscode.commands.registerCommand('timer-extension.start-timer', () => {
        const panel = vscode.window.createWebviewPanel('timer', 'Pomodoro Timer', vscode.ViewColumn.Two, {
            enableScripts: true,
        });
        function updateWebview() {
            panel.webview.html = `
